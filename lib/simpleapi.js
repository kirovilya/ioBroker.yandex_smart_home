...
function SimpleAPI(server, webSettings, adapter, instanceSettings, app) {
    ...
    this.restApi = function (req, res, isAuth, isChecked) {
        ...
        switch (command) {
            ...
            case '/auth':
                status = 200;
                const authResult = '<meta name="viewport" content="width=device-width, height=device-height initial-scale=1 user-scalable=no">' +
                '<body style="background:#310d80;">' +
                '<footer style="position:fixed; bottom:0; left:0; right:0; padding:24px 16px;">' +
                '<a style="display:block; text-align:center; background:#7b3dcc; color:#fff; cursor:pointer; font-family:Arial,sans-serif; font-size:20px; padding:13px 16px; border-radius:6px; text-decoration:none;" href="' + values.redirect_uri + '?code=test123&state=' + values.state + '">Подключить умный дом</a>';
                doResponse(res, 'plain', status, headers, authResult, false);
                break;
